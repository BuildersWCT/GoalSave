...
async function main() {
    ...
    const [upgrader] = await hardhat_1.ethers.getSigners();
    ...
    const CeloSaveV2 = await hardhat_1.ethers.getContractFactory("CeloSave");
    console.log("\n🔄 Upgrading to new implementation...");
    console.log("This will use the timelock mechanism if configured");
    const upgraded = await hardhat_1.upgrades.upgradeProxy(proxyAddress, CeloSaveV2);
    await upgraded.waitForDeployment();
    const newImplementationAddress = await hardhat_1.upgrades.erc1967.getImplementationAddress(proxyAddress);
    console.log("\n✅ Upgrade Successful!");
    console.log("─────────────────────────────────────────");
